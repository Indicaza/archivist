...
Item {
    ...
    readonly property bool activeTabDirty: {
        var index = tabIndexForKey(activeTabKey)
        return index >= 0
            ? Boolean(tabs.get(index).dirty)
            : false
    }
    ...
    function tabStateArray() {
        ...
        for (var index = 0; index < tabs.count; index += 1) {
            var snapshot = tabSnapshot(index)
    ...
    function setDocumentDirty(documentId, dirty) {
        var targetId = String(documentId || "")
        ...
        for (var index = 0; index < tabs.count; index += 1) {
            ...
            var tabDocumentId = collectionId
                + ":"
                + String(tab.libraryId || "")
                + ":"
                + String(tab.fileId || "")

            if (tabDocumentId === targetId) {
                tabs.setProperty(
                    index,
                    "dirty",
                    Boolean(dirty)
    ...
    ListView {
        ...
        delegate: Item {
            ...
            Button {
                ...
                contentItem: Text {
                    text: tabItem.dirty ? "●" : parent.text
